...
- **Endpoint**: Đường dẫn cụ thể mà frontend/backend gọi (ví dụ: `/chat`).
...
- **Endpoint chính:**
  - `POST /chat` — Nhận message, xử lý toàn bộ pipeline.
...
- Frontend gửi request HTTP (POST) đến `/chat`.

### 2. API Gateway nhận request
- Tìm router phù hợp với đường dẫn (`/chat`).
- Router này được định nghĩa trong `chatbot_api.py` với prefix `/api/v1`.

### 3. Endpoint `/chat` xử lý:
...
|-----------------|--------------------------|----------------------------------------|
| API Gateway     | `/chat`           | Xử lý hội thoại, điều phối pipeline    |
...
- **Luồng đi chuẩn:**
  - Frontend → API Gateway (`/chat`) → [Gating Router, các service phân tích] → Model Server (`/generate`) → API Gateway → Frontend
